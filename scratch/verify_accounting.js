...
async function checkAccounting() {
    ...
    transactions.forEach(tx => {
        ...
        const isRefund = desc.includes('refund') || tx.type?.toLowerCase().includes('refund');

        if (isRefund) {
            stats.refunds += amount;
        } else {
            if (desc.includes('rent')) stats.rent += amount;
            else if (desc.includes('deposit')) stats.deposits += amount;
            else if (desc.includes('fee')) stats.fees += amount;
        }
    });
